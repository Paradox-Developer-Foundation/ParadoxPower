...
module UtilityParser =
    ...
    let loadEventTargetLinks anyScope parseScope allScopes filename fileString =
        ...
        | Success(s, _, _) ->
            let root =
                ParadoxPower.Process.ProcessCore.simpleProcess.ProcessNode () "root" (mkZeroFile filename) s
    ...
    let private loadScopeDefinitions filename fileString =
        ...
        | Failure(e, _, _) ->
            ...
            ([], [])
        | Success(s, _, _) ->
            let root = ParadoxPower.Process.ProcessCore.simpleProcess.ProcessNode () "root" (mkZeroFile filename) s
